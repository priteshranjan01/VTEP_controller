...
from ryu.ofproto.ofproto_v1_4_parser import OFPActionPushVlan
...
class Switch(object):
    ...
    def __repr__(self):
        return "Switch: type= {0}, dpid= {1}, host_ip= {2}, vni_to_local_port=" \
               "{3}, vni_to_vxlan_port={4} vni_to_vlan={5}".format(
                   self.type, hex(
                       self.dpid), self.host_ip, self.vni_to_local_port,
                   self.vni_to_vxlan_port, self.vni_to_vlan)
...
class VtepConfigurator(app_manager.RyuApp):
    ...
    def _read_config(self, file_name="CONFIG.json"):
        ...
        for dp in config_data['switches']:
            switch_type = VXLAN_ENABLED if dp['type'] == 'VXLAN_ENABLED' \
                else VXLAN_GATEWAY
            dpid = int(dp['id'], 16)
            host_ip = dp['host_ip']
            ...
            if switch_type == VXLAN_ENABLED:
                ...
                for vni, values in dp["vni_to_vlan_and_vxlan_port"].items():
                    ...
                    switch.vni_to_vlan.update({int(vni): map(
                        int, vlanID.split(','))[0]})
                    switch.vni_to_vxlan_port.update({int(vni): map(
                        int, vxlan_ports.split(','))})
                switch.vlanId_to_trunk_port = {
                    int(vlanID): map(int, ports) for vlanID, ports in
                    dp['vlan_to_trunk_port'].items()}
    ...
    def _connection_up_handler(self, ev):
        def _add_default_resubmit_rule(next_table_id=1):
            # Adds a low priority rule in table 0 to resubmit the unmatched
            # packets (i.e. the packets which didn't come from local port) to
            # table 1
            ...
            print("{0} : {1} : Rule added, table={2} priority={3} resubmit={"
                  "4}".format(dpid_hex, st, 0, 0, next_table_id))

            # Add a low priority rule in table 1 to forward table-miss to
            # controller. These will cause a Packet_IN at controller
            ...
            mod = parser.OFPFlowMod(datapath=datapath, table_id=1, priority=0,
                                    match=match, instructions=inst)
            st = datapath.send_msg(mod)
            print("{0} : {1} : Rule added, table={2} priority={3} "
                  "Forward to CONTROLLER".format(dpid_hex, st, 1, 0))
        ...
        # Switch will contain all the information from CONFIG about this
        # particular datapath
        ...
        if switch.type == VXLAN_ENABLED:
            for vni, ports in switch.vni_to_local_port.items():  #
                ...
                for port in ports:
                    ...
                    # These rules will ensure that all the packets coming from
                    # local ports have tunnel_id associated with them, when
                    # packet processing reaches table 1.
                    ...
                    mod = parser.OFPFlowMod(datapath=datapath, priority=100,
                                            match=match, instructions=inst)
                    st = datapath.send_msg(mod)
                    print ("{0} : {1} : Rule added, match(in_port={2}) "
                           "set_tun_id={3}, resubmit({4}".format(dpid_hex, st,
                                                                 port, vni, 1))

        elif switch.type == VXLAN_GATEWAY:
            for vni, vlan in switch.vni_to_vlan.items():
                # Add a rule to stip VLAN ID, Add a corresponding VNI and
                # resubmit to 1. These rules will ensure that all the packets
                # coming from local ports have VLAN tag stripped off and
                # corresponding tun_id associated.
