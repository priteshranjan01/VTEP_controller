...
class VtepConfigurator(app_manager.RyuApp):
        ...
        for dp in config_data['switches']:
            ...
            if switch_type == VXLAN_ENABLED:
                for vni, ports in dp["vni_to_local_and_vxlan_port"].items():
                    ...
                    switch.vni_to_local_port.update({int(vni): map(int, local_ports.split(','))})
                ...
                for vni, values in dp["vni_to_vlan_and_vxlan_port"].items():
                    ...
                    switch.vni_to_vlan.update({int(vni): map(int, vlanID.split(','))[0]})
                    switch.vni_to_vxlan_port.update({int(vni): map(int,vxlan_ports.split(','))})
                #pdb.set_trace()
                switch.vlanId_to_trunk_port= {int(vlanID): map(int, ports) for vlanID, ports in dp['vlan_to_trunk_port'].items()}
    ...
    def _connection_up_handler(self, ev):
        def _add_default_resubmit_rule(next_table_id=1):
            ...
            print("{0} : {1} : Rule added, table={2} priority={3} resubmit={4}".format(dpid_hex, st, 0, 0, next_table_id))
            ...
            st = datapath.send_msg(mod)
            print("{0} : {1} : Rule added, table={2} priority={3} Forward to COTROLLER".format(dpid_hex, st, 1, 0))
        ...
        if switch.type == VXLAN_ENABLED:
            for vni, ports in switch.vni_to_local_port.items():  # (e.g. vni=101 and ports= [1,4])
                for port in ports:
                    ...
                    st = datapath.send_msg(mod)
                    print ("{0} : {1} : Rule added, match(in_port={2}) set_tun_id={3}, resubmit({4}".format(dpid_hex, st, port, vni, 1))
            ...
            for vni, vlan in switch.vni_to_vlan.items():
                ...
                match = parser.OFPMatch(vlan_vid=(0x1000 | vlan))  # vlan is a list with just one item
                ...
                mod = parser.OFPFlowMod(datapath=datapath, priority=100,
                                        match=match, instructions=inst)
                st = datapath.send_msg(mod)
                print("{0}: {1}: Rule Added, match(vlan={2}) pop_vlan, set_tun_id={3} resubmit={4}".format(dpid_hex, st, vlan, vni, 1))
    ...
    def _packet_in_handler(self, ev):
        ...
        # Only those packets will come to controller which have tunnel_id already set.
        vni = msg.match['tunnel_id']
        print ("{0} Received a packet on port={1} of VNI ID={2} from eth_src={3} to eth_dst={4}".format(dpid_hex, in_port, vni, eth.src, eth.dst))
            ...
            if eth.dst == L2_BROADCAST:
                ...
                if in_port in vxlan_ports:  # Incoming traffic
                    for port in local_ports:  # Multicast on each local ports
                        ...
                        print("{0}: {1} Packet src={2}, destination={3}, output={4}".format(dpid_hex, st, eth.src, eth.dst, port))
                ...
                if in_port in vxlan_ports:  # Incoming unicast message
                    ...
                    actions = [parser.NXActionSetTunnel(tun_id=vni), parser.OFPActionOutput(port=out_port)]
                    out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER, in_port=in_port,
                                              actions=actions, data=pkt)
                    st = datapath.send_msg(out)
                    print("{0} : {1} Outgoing traffic. setTunnelId={2} out_port={3}".format(
            ...
            if eth.dst == L2_BROADCAST:
                ...
                if in_port in vxlan_ports:  # Incoming traffic
                    ...
                    for port in trunk_ports:  # Output on all ports that are on VLAN networks
                        ...
                        out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER, in_port=in_port,
                                                  actions=actions, data=pkt)
                        st = datapath.send_msg(out)
                        print("{0}: {1} Packet out. Actions=Set_VLAN_id={2}, out_port={3}".format(dpid_hex, st, vlan_id, port))
                    ...
                    for port in vxlan_ports:
                        ...
                        out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER, in_port=in_port,
                                                  actions=actions, data=pkt)
                        st = datapath.send_msg(out)
                        print("{0}: {1} Outgoing traffic output. setTunnelID={2} out_port={3}".format(dpid_hex, st, vni, port))
                ...
                if in_port in vxlan_ports:  # Incoming traffic
                    ...
                    out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER, in_port=in_port,
                                                  actions=actions, data=pkt)
                    st = datapath.send_msg(out)
                    print("{0}: {1} Packet Output. SetVlanID:{2} output={3}".format(dpid_hex, st, vlan_id, out_port))

                else:  # outgoing traffic
                    out_port = switch.mac_vni_to_port[(eth.dst, vni)]
                    # Add a unicast rule for traffic forward from trunk_port to vxlan_port
                    match = parser.OFPMatch(tunnel_id=vni, eth_dst=eth.dst)
                    actions = [parser.OFPActionOutput(port=out_port)]
                    inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
                    mod = parser.OFPFlowMod(datapath=datapath, table_id=1, priority=100, match=match,
                    ...
                    print("{0} : {1} Rule added table=1, priority=100, match(tunnel_id={2}, eth_dst={3}, out_port={4}".format(
                            dpid_hex, st, vni, eth.src, out_port))
                    # Add a unicast rule for traffic forward from vxlan_port to trunk_port
                    match = parser.OFPMatch(tunnel_id=vni, eth_dst=eth.src)
                    actions = [OFPActionPushVlan(ether_types.ETH_TYPE_8021Q),
                               OFPActionSetField(vlan_vid=(0x1000 | vlan_id)),
                    ...
                    out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER, in_port=in_port,
                                              actions=actions, data=pkt)
                    st = datapath.send_msg(out)
                    print("{0}: {1} Packet out. Set tun_id={2} out_put_port={3}".format(dpid_hex, st, vni, out_port))
